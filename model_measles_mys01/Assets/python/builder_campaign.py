...
def campaignBuilder():
  ...
  COVID_FAC    = gdata.var_params['R0_covid_fac']
  ...
  # Add MCV1 RI
  mcv1_dat = np.loadtxt(os.path.join('Assets','data', 'MYS_MCV1.csv'), delimiter=',')
  with open(os.path.join('Assets','data', 'MYS_MCV1.json')) as fid01:
    mcv1_dict = json.load(fid01)

  TIME_START = 365.0*(START_YEAR-BASE_YEAR)
...
def IV_ReduceR0(params=dict()):
  ...
  camp_event.Event_Coordinator_Config       = camp_coord
  camp_event.Start_Day                      = 120.0*365.0 +90.0
  camp_event.Nodeset_Config                 = node_set
  ...
  camp_iv01.Multiplier_By_Duration.Times      = [0.0*365.0,
                                                 4.0*365.0]
  camp_iv01.Multiplier_By_Duration.Values     = [params['mult_fac'],
                                                 params['mult_fac']]
