#*******************************************************************************

import os, sys, json

import numpy              as np
import matplotlib.pyplot  as plt
import matplotlib.patches as patch
import matplotlib         as mpl

#*******************************************************************************

DIRNAME = 'experiment_meas_gha_calib04'
YMAX    =  2500

targfile = os.path.join('..',DIRNAME,'param_dict.json')
with open(targfile) as fid01:
  param_dict = json.load(fid01)

targfile = os.path.join('..',DIRNAME,'data_brick.json')
with open(targfile) as fid01:
  data_brick = json.load(fid01)

nsims    = int(param_dict['NUM_SIMS'])

tvals    = data_brick.pop('tstamps')
ntstp    = len(tvals)
infBlock = np.zeros((nsims,ntstp))
sclVal   = np.zeros((nsims,1))

for sim_idx_str in data_brick:
  try:
    sim_idx = int(sim_idx_str)
  except:
    continue
  infDat = np.array(data_brick[sim_idx_str]['timeseries'])
  infBlock[sim_idx,:] = infDat
  sclVal[sim_idx,0]     = data_brick[sim_idx_str]['rep_rate']

# Scaling factor
infBlock = infBlock * sclVal


# Figure
fig01 = plt.figure(figsize=(8,6))

axs01  = fig01.add_subplot(111)
plt.sca(axs01)

axs01.grid(visible=True, which='major', ls='-', lw=0.5, label='')
axs01.grid(visible=True, which='minor', ls=':', lw=0.1)
axs01.set_axisbelow(True)

yval = np.median(infBlock,axis=0)
xval = np.array(tvals)/365+1900

xv1 = np.array(param_dict['EXP_VARIABLE']['log10_import_rate'])
xv2 = np.array(param_dict['EXP_VARIABLE']['R0_peak_magnitude'])
xv3 = np.array(param_dict['EXP_VARIABLE']['SIA_cover_GHA-2013'])

gidx = (xv2 > 1.2) & (xv2 < 1.4) & (xv3 < 0.6) & (xv1 < -0.6)

print(np.sum(gidx))

for k1 in range(infBlock.shape[0]):
  if(gidx[k1] == 1):
    axs01.plot(xval,infBlock[k1,:],color='C0',linewidth=2)

axs01.plot(xval,np.median(infBlock[gidx,:],axis=0),color='k',linewidth=2)



axs01.set_ylabel('Monthly Infections',fontsize=14)
axs01.set_xlim(2010, 2020)
#axs01.set_ylim(   0, YMAX)
axs01.tick_params(axis='x', labelsize=16)

plt.savefig('fig_clouds41.png')

plt.close()

#*******************************************************************************

