...
# Parse CSVs
wpp_file1    = r'WPP2019_POP_F07_1_POPULATION_BY_AGE_BOTH_SEXES_ESTIMATES.csv'
with open(wpp_file1, errors='ignore') as fid01:
  flines_rev = [val.strip().split(',') for val in fid01.readlines()]

wpp_file2    = r'WPP2019_POP_F07_1_POPULATION_BY_AGE_BOTH_SEXES_MEDIUM_VARIANT.csv'
...
for row_val in flines_rev:
  if(reprule(row_val[2]) == tlc_dict[COUNTRY]):
    year_val = int(row_val[7])
    bin_pops = [int(1000*float(row_val[idx])) for idx in range(8,29)]
