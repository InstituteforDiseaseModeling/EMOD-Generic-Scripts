...
def demographicsBuilder():
  ...
  POP_INIT     = gdata.var_params['num_agents']
  ...
  pop_ref   = [np.interp(year_init, year_vec, pop_mat[idx,:]) for idx in range(pop_mat.shape[0])]


  # ***** Populate nodes in primary file *****
  node_list = list()

  gdata.init_pop  = POP_INIT
  node_id         = 1
  node_name       = 'EXAMPLE:A{:05d}'.format(node_id)
  ...
  # ***** Create primary file *****
  REF_NAME   = 'Example_Demographics_Datafile'
  ...
  # Enforce steady-state demography
  ...
  age_y        = pop_age_days
  age_init_cdf = np.cumsum(pop_ref[:-1])/np.sum(pop_ref)
