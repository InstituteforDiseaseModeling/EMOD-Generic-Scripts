...
def calibration_daemon():
  ...
  for iter_num in range(param_dict['NUM_ITER']):
    ...
    param_dict_new['EXP_NAME'] = EXP_BASE_NAME + '_iter{:02d}'.format(iter_num)
    ...
    PATH_EXP_DEF = os.path.abspath('param_dict.json'.format(iter_num))
    ...
    write_id_file('COMPS_ID_iter{:02d}.id'.format(iter_num), exp_obj_new)

    # Save reduced data to calibration object
    pool_manager(exp_obj_new.id)
    os.rename('data_brick.json','data_brick_iter{:02d}.json'.format(iter_num))
    os.rename('param_dict.json','param_dict_iter{:02d}.json'.format(iter_num))
