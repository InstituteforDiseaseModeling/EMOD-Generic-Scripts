...
from dtk_post_process import gha_2010_frac, gha_2020_frac
...
for k1 in range(2, pyr_mat_avg.shape[0], 10):
  ...
  axs01 = fig01.add_subplot(int('22{:d}'.format(int((k1-2)/10)+1)), label=None)
  plt.sca(axs01)
  ...
  axs01.text( -7, 92.5, '{:04d}'.format(2008+k1), fontsize=24)
  axs01.text(  3, 87.5, 'Total Pop\n{:6.3f}M'.format(tpop/1e6), fontsize=18)

# Figures - Reference
for k1 in range(len(x_dat_list)):

  axs01 = fig01.add_subplot(int('22{:d}'.format(k1+2+1)), label=None)
  plt.sca(axs01)
  ...
  axs01.text( -7, 92.5, '{:04d}'.format(2010+10*k1), fontsize=24)
